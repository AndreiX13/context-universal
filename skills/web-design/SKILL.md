---
name: web-design
description: "Anti-AI-slop design skill for marketing and landing pages — greenfield pages, audits, redesigns, and design extraction from URLs or screenshots. Use when the user asks to build a new landing page, marketing site, or app shell, wants to redesign something, or uses audit/redesign/study. For product/application UI (dashboards, admin, settings, data tables) use interface-design instead. Shares the project-wide quality bar in canon/ (anti-slop, motion, typography, color, copy)."
metadata:
  source: "Hallmark by Together AI / Nutlope (MIT), branding removed"
  scope: universal
---

# web-design (anti-slop marketing & landing pages)

A design skill for AI coding assistants. Makes the UIs they generate look made, not generated.

This skill is opinionated, short, and boring on purpose. It encodes a tight set of rules — drawn from the consensus of the anti-AI-slop design field — and refuses to let the model fall back to the defaults every LLM was trained on.

The differentiator: it insists on **structural variety**, not just visual variety. Two pages for two different briefs should not share the same hero → 3-feature → CTA → footer rhythm. They should feel like different sites, not colour-swaps of the same template. See [`references/structure.md`](references/structure.md).

> **Project canon.** The cross-cutting rules this skill enforces (anti-slop tells, motion, typography, color, copy) also live — deduplicated and authoritative — in the repo's `canon/` directory. When this skill and `canon/` disagree, `canon/` wins. This skill adds the marketing-page-specific structure (macrostructures, component archetypes, genres, themes) on top of that shared floor.
...
- [`study.md`](references/study.md) — load only when the `study` verb runs.
